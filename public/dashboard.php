...
if ($user_type == 'dpjp'){
  $sql = "SELECT * FROM pemeriksaan WHERE ID_USER = '$user_id'";
  $q = mysqli_query($connect, $sql);
  $sqlex = "SELECT * FROM pemeriksaan WHERE ID_USER = '$user_id' AND expertise IS NOT NULL AND expertise != ''";
  $qex = mysqli_query($connect, $sqlex);
} else {
  $sql = "SELECT * FROM pemeriksaan";
  $q = mysqli_query($connect, $sql);
  $sqlex = "SELECT * FROM pemeriksaan WHERE expertise IS NOT NULL AND expertise != ''";
  $qex = mysqli_query($connect, $sqlex);
}

$last = "SELECT * FROM pemeriksaan ORDER BY ID_PEMERIKSAAN DESC LIMIT 1";
$queri_last = mysqli_query($connect, $last);
$p_last = mysqli_fetch_array($queri_last);
?>
<!DOCTYPE html>
<html lang="en">
          ...
          <div class="bg-white border rounded-xl shadow-lg p-6 pt-4 w-3/12 min-w-[20rem]">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="flex flex-col items-center justify-center bg-yellow-100 rounded-lg p-4">
                ...
                <p class="text-2xl font-bold text-gray-800"><?php echo mysqli_num_rows($q); ?></p>
              ...
              <div class="flex flex-col items-center justify-center bg-green-100 rounded-lg p-4">
                ...
                <p class="text-2xl font-bold text-gray-800"><?= $p_last['ID_PEMERIKSAAN']  ?></p>
